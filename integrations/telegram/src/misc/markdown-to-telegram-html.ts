...
// markdown-it's `Token`/`Options` are not named ESM exports under our Bundler moduleResolution (the
// donor relied on `Token` namespace access, which only resolves under node10 resolution).
// Derive them from the instance's render-rule signature — identical types, resolution-independent.
type MdInstance = ReturnType<typeof MarkdownIt>
type RenderRule = NonNullable<MdInstance['renderer']['rules'][string]>
type Token = Parameters<RenderRule>[0][number]
type Options = Parameters<RenderRule>[2]
...
md.renderer.rules.link_open = ruleHandler((token: Token) => {
  ...
  const formattedAttributes = token.attrs?.reduce(
    (formattedAttrs: string, [attrName, attrValue]: [string, string]) => `${formattedAttrs} ${attrName}="${attrValue}"`,
    ''
  )
