...
Запускается на нашем runtime-host по той же модели, что telegram/megaplan (Botpress
`@botpress/sdk`). Интеграция предоставляет только **actions** — входящих вебхуков нет.
