...
import { BotpressClientLike, Cognitive, Model, Models } from '@holocronlab/botruntime-cognitive'

import { type TextTokenizer, getWasmTokenizer } from '@holocronlab/botruntime-thicktoken'
...
export class Zai {
  ...
  protected async getTokenizer() {
    Zai.tokenizer ??= await (async () => {
      ...
      return (await getWasmTokenizer()) as TextTokenizer
    })()
