...
export const parseReq = (input: ListFilesInput): ListFilesRequest & { path: string } => {
  return {
    path: `/v1/files`,
    headers: {  },
    query: { 'nextToken': input['nextToken'], 'pageSize': input['pageSize'], 'sortField': input['sortField'], 'sortDirection': input['sortDirection'], 'tags': input['tags'], 'ids': input['ids'], 'indexed': input['indexed'] },
    params: {  },
    body: {  },
  }
}
