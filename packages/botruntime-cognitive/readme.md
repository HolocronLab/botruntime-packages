...
If no provider is passed to Cognitive, we default to `RemoteModelProvider`, which uses the File API to store preferences and fetches models using the botruntime Client (`getBot` and `listLanguageModels` on each installed integrations).
...
```ts
import { Cognitive, ModelProvider } from '@holocronlab/botruntime-cognitive'
